...
use App\Http\Controllers\V1\AngleOneAccount;
...
Route::post('angle-one/postback/postback/{slug}', function (\Illuminate\Http\Request $request, $slug) {
  \Illuminate\Support\Facades\Log::info('postback webhook', [$slug, $request->all()]);
...
Route::middleware('guest')->group(function () {
  ...
  Route::get('resend-forget-password', [ForgotPasswordController::class, 'sendResetLinkEmail'])
...
Route::middleware('auth')->group(function () {
  ...
  Route::prefix('angle-one')->name('angle-one.')->group(function () {
    ...
    Route::post('account/resend-totp-otp/{account}', [AngleOneAccount::class, 'resendTotpOtp'])->name('resend.totp.otp');
